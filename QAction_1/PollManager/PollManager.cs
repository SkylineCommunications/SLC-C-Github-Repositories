namespace Skyline.Protocol.PollManager
{
    ...
    public static class PollManager
    {
        ...
        {
            { RequestType.Repositories_Repositories, new PollSettings { PollFequency = TimeSpan.FromMinutes(10), Enabled = true } },
            { RequestType.Repositories_Tags, new PollSettings { PollFequency = TimeSpan.FromMinutes(360), Enabled = true } },
            { RequestType.Repositories_Releases, new PollSettings { PollFequency = TimeSpan.FromMinutes(360), Enabled = true } },
            { RequestType.Repository_Issues, new PollSettings { PollFequency = TimeSpan.FromMinutes(5), Enabled = true } },
            { RequestType.Repositories_Workflows, new PollSettings { PollFequency = TimeSpan.FromMinutes(360), Enabled = true } },

            { RequestType.Organizations_User, new PollSettings { PollFequency = TimeSpan.FromHours(24), Enabled = true } },
            { RequestType.Organizations_Repositories, new PollSettings { PollFequency = TimeSpan.FromHours(10), Enabled = true } },
        };
        ...
        public static Dictionary<RequestType, PollSettings> InitPollDictionary(SLProtocol protocol)
        {
            ...
            {
                dic.Add((RequestType)pollManagerKeys[i], new PollSettings { Enabled = pollManagerStates[i], PollFequency = pollManagerFrequencies[i], LastPollTime = pollManagerLastPolled[i] });
            }
            ...
            foreach (KeyValuePair<RequestType, PollSettings> kvp in InitialSettings)
            {
                pollManagerRows.Add(
                    new PollmanagerQActionRow
                    {
                        ...
                        Pollmanagerpollfrequency_21004 = kvp.Value.PollFequency.TotalSeconds,
        ...
        public static void PollDeviceObjects(SLProtocol protocol, Dictionary<RequestType, PollSettings> pollItems, DateTime utcNow)
        {
            ...
            foreach (KeyValuePair<RequestType, PollSettings> pollItem in pollItems.OrderBy(x => (int)x.Key))
            {
                if (!pollItem.Value.Enabled
                    || pollItem.Value.LastPollTime + pollItem.Value.PollFequency > utcNow)
                {
                    continue;
                }
        ...
        public sealed class PollSettings
        {
            ...
            public TimeSpan PollFequency { get; set; }
        }
    }
}