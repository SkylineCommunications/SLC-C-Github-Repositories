	...
	internal static class WorkflowValidation
	{
		public static bool Validate(AddWorkflowRequest workflowRequest, RepositoriesModel repo, out string result)
		{
			...
			switch (workflowRequest)
			{
				...
				case AddCustomWorkflowRequest customWorkflowRequest:
					success = ValidateCustom(customWorkflowRequest, out result);
					break;
		...
		private static bool ValidateCustom(AddCustomWorkflowRequest workflowRequest, out string result)
		{
			result = String.Empty;

			// Check Yaml
			if (String.IsNullOrWhiteSpace(workflowRequest.Workflow.WorkflowYaml))
			{
				result = "The workflow yaml cannot be left blank.";
				return false;
			}

			if (!YamlConvert.IsValidYaml(workflowRequest.Workflow.WorkflowYaml))
			{
				result = "The workflow yaml is not valid.";
				return false;
			}

			// Check Nuget API key
			if (String.IsNullOrWhiteSpace(workflowRequest.Workflow.WorkflowName))
			{
				result = "The workflow name cannot be left blank";
				return false;
			}
